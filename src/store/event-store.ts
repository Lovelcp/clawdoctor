...
import type { ClawDoctorEvent, EventType } from "../types/events.js";
...
function rowToEvent(row: EventRow): ClawDoctorEvent {
  return {
    id: row.id,
    source: row.source,
    ...
    data: JSON.parse(row.data) as ClawDoctorEvent["data"],
  };
}
...
export interface EventStore {
  insertEvent(event: ClawDoctorEvent): void;
  queryEvents(filter: EventFilter): ClawDoctorEvent[];
  /**
   ...
   */
  queryEventsWithSourcePriority(filter: SourcePriorityFilter): ClawDoctorEvent[];
}
...
export function createEventStore(db: Database.Database): EventStore {
  ...
  function insertEvent(event: ClawDoctorEvent): void {
    insertStmt.run({
      id: event.id,
      source: event.source,
  ...
  function queryEvents(filter: EventFilter): ClawDoctorEvent[] {
  ...
  function queryEventsWithSourcePriority(filter: SourcePriorityFilter): ClawDoctorEvent[] {
