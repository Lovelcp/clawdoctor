  ...
  for (const plugin of plugins) {
    ...
    for (const disease of plugin.diseases) {
      if (builtinIds.has(disease.id)) {
        console.warn(
          `[clawdoctor] Plugin "${plugin.name}" disease "${disease.id}" conflicts with a built-in disease ID. Skipping.`,
        );
        continue;
      }
      if (extra.some((e) => e.id === disease.id)) {
        console.warn(
          `[clawdoctor] Plugin "${plugin.name}" disease "${disease.id}" conflicts with another plugin's disease ID. Skipping.`,
        );
        continue;
      }
      extra.push(disease);
