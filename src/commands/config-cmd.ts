// ═══════════════════════════════════════════════
//  Config Command
//  clawdoctor config init | set <key> <value> | show
// ═══════════════════════════════════════════════
...
export function registerConfigCommand(program: Command): void {
  const config = program
    .command("config")
    .description("Manage ClawDoctor configuration");

  // clawdoctor config init
  config
    .command("init")
    .description("Create ~/.clawdoctor/config.json with default settings")
    .action(() => {
      try {
  ...
  // clawdoctor config set <key> <value>
  config
  ...
  config
    ...
    .action(() => {
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        console.error(`[clawdoctor] config show failed: ${message}`);
        process.exit(1);
      }
    });
}
