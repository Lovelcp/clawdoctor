# ClawDoctor Phase 2 Implementation Plan (v2)

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extend ClawDoctor with LLM-powered deep diagnosis (16 hybrid/LLM diseases + cross-department causal chains), a full prescription lifecycle (generate/preview/apply/rollback/follow-up), a Web Dashboard (Hono + SPA with 9 pages), and an OpenClaw Plugin for real-time event streaming.
...
**Decision:** `clawdoctor checkup` ALWAYS writes to `~/.clawdoctor/clawdoctor.db` (persistent).
...
```
clawdoctor checkup     → writes to ~/.clawdoctor/clawdoctor.db
clawdoctor rx list     → reads from ~/.clawdoctor/clawdoctor.db
clawdoctor dashboard   → reads from ~/.clawdoctor/clawdoctor.db
plugin mode         → writes to ~/.clawdoctor/clawdoctor.db (same file)
```

`CheckupOptions` gets a `dbPath?: string` (default: `~/.clawdoctor/clawdoctor.db`). For tests, pass `:memory:` explicitly. The `db?: Database.Database` from the previous plan is REMOVED — it created lifecycle ambiguity. Instead, the pipeline always opens/closes its own DB at the given path.
...
- Token is injected into the SPA via `window.__CLAWDOCTOR_TOKEN__` in a `<script>` tag that the server injects before serving index.html
- SPA reads `window.__CLAWDOCTOR_TOKEN__` and attaches `Authorization: Bearer <token>` to every `/api/*` fetch
...
app.get("/", (c) => {
  const html = SPA_HTML.replace(
    "</head>",
    `<script>window.__CLAWDOCTOR_TOKEN__="${opts.authToken}";</script></head>`
  );
  return c.html(html);
});
...
├── commands/
│   ├── rx-cmd.ts                  # clawdoctor rx list/preview/apply/rollback/followup/history
│   └── dashboard-cmd.ts           # clawdoctor dashboard [--port]
├── scripts/
...
export function resolveLLMProvider(config: ClawDoctorConfig): LLMProvider | null {
...
**Standalone dashboard data flow:** Dashboard reads from `~/.clawdoctor/clawdoctor.db` (same file that `clawdoctor checkup` writes to). If no recent data exists, it runs a fresh checkup first:
...
Three methods:
- `getRecentSessionSamples(agentId, limit)`: Parse JSONL files, extract tool call sequences with error summaries (first 200 chars, redacted). NOT the same as session-parser.ts which produces ClawDoctorEvent with paramsSummary.
...
export const clawdoctorPlugin: OpenClawPluginDefinition = {
  ...
  register(api) {
    ...
    // 6. Register CLI subcommands (openclaw clawdoctor checkup)
...
Replace the Phase 1 `:memory:` default with `~/.clawdoctor/clawdoctor.db`:
...
export interface CheckupOptions {
  ...
  noLlm: boolean;
  configPath?: string;
  dbPath?: string;  // default: ~/.clawdoctor/clawdoctor.db. Use ":memory:" for tests only.
}

// In runCheckup():
const dbDir = join(opts.stateDir ?? join(homedir(), ".clawdoctor"));
...
```typescript
// Detects persistent DB at ~/.clawdoctor/clawdoctor.db or falls back to :memory:
...
1. `clawdoctor checkup --json` includes `llmAvailable` and `llmDegradationReason` fields
2. `clawdoctor rx list --json` returns array (empty when no prescriptions)
3. `clawdoctor dashboard --help` shows --port option
...
9. **Cross-command data flow:** Run `checkup --json`, then `rx list --json` — verify prescriptions from checkup are visible in rx list (both read from same ~/.clawdoctor/clawdoctor.db)
