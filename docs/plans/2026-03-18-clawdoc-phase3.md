# ClawDoc Phase 3 Implementation Plan — Community Virality Edition

> **For agentic workers:** Use superpowers:subagent-driven-development for parallel execution.

**Goal:** Deliver 5 high-virality features: Skill Quality Badge, CI integration, community disease plugins, auto-prescription, and npm publish with docs. Transform ClawDoc from "a tool" into "a platform with a viral loop."
...
- **CLI:** `clawdoc badge` outputs SVG to stdout or file
...
Badge format: `ClawDoc | A 95` with grade-colored right half (green A, amber C, red F).

No external badge service dependency — badges are self-generated SVGs.

### 2. CI Integration

`clawdoc checkup --fail-on <severity>` sets process.exitCode based on findings:
...
Convention: npm packages named `clawdoc-plugin-*` or `@scope/clawdoc-plugin-*`.
...
Loading: `clawdoc checkup --plugins clawdoc-plugin-security-extra` or `config.plugins: ["clawdoc-plugin-security-extra"]`.

At startup, ClawDoc `import()`s each plugin, merges its diseases into the registry, and registers custom rule evaluators.

### 4. Auto-Prescription

`clawdoc checkup --auto-fix` automatically applies all `guided` prescriptions with `risk: "low"`.
...
describe("BadgeGenerator", () => {
  it("generates valid SVG for grade A", () => {
    const svg = generateBadge({ grade: "A", score: 95, label: "ClawDoc" });
  ...
  it("uses red color for grade F", () => {
    const svg = generateBadge({ grade: "F", score: 15, label: "ClawDoc" });
    expect(svg).toContain("#e05d44"); // shields.io red
  });

  it("uses gray for N/A", () => {
    const svg = generateBadge({ grade: "N/A", score: 0, label: "ClawDoc" });
    expect(svg).toContain("#9f9f9f");
  });

  it("produces valid XML", () => {
    const svg = generateBadge({ grade: "B", score: 78, label: "ClawDoc" });
...
```bash
clawdoc badge                        # print SVG to stdout
clawdoc badge --output badge.svg     # save to file
...
```json
{
  "keywords": ["openclaw", "diagnostics", "health", "agent", "ai", "lobster", "clawdoc"],
  "homepage": "https://github.com/openclaw/clawdoc",
  "repository": { "type": "git", "url": "https://github.com/openclaw/clawdoc.git" },
  "bugs": { "url": "https://github.com/openclaw/clawdoc/issues" },
...
How to write a ClawDoc disease plugin: plugin API, DiseaseDefinition format, publishing to npm.
...
Step-by-step guide to creating a `clawdoc-plugin-*` package.
...
describe("E2E Phase 3", () => {
  ...
  it("clawdoc checkup --fail-on critical exits 0 with no critical diseases", () => {
