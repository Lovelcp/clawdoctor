# ClawDoctor Phase 2 Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extend ClawDoctor with LLM-powered deep diagnosis (16 hybrid/LLM diseases + cross-department causal chains), a full prescription lifecycle (generate/preview/apply/rollback/follow-up), a Web Dashboard (Hono + Preact SPA with 9 pages), and an OpenClaw Plugin for real-time event streaming.
...
**Spec:** `docs/2026-03-17-clawdoctor-design.md` — sections §5.3 (Stream Collector), §6.1.1 (RawSampleProvider), §6.4 (LLM Analyzer), §6.5 (Cross-Department Linker), §7 (Prescription System), §9.3 (Web Dashboard).
...
// Resolve LLM config: OpenClaw config → ClawDoctor config override → env var fallback
export function resolveLLMConfig(config: ClawDoctorConfig): LLMClientOptions | null {
...
GET  /api/events           → ClawDoctorEvent[] (paginated: ?page=1&limit=50&type=tool_call)
...
export const clawdoctorPlugin: OpenClawPluginDefinition = {
  ...
  register(api) {
    ...
    // 5. Register CLI subcommands (openclaw clawdoctor checkup)
    // 6. Register dashboard HTTP route (/clawdoctor/*)
    // 7. Register follow-up scheduler service (spec §7.5):
    //    - api.registerService({ id: "clawdoctor-followup", start: interval timer, stop: clearInterval })
...
Note: For the OpenClaw plugin types, create a minimal `src/plugin/openclaw-types.ts` with just the interfaces needed (OpenClawPluginDefinition, OpenClawPluginApi) — don't import from OpenClaw directly since ClawDoctor is a standalone package.
...
```typescript
export function createPrescriptionExecutor(db: Database, config: ClawDoctorConfig) {
  return {
...
- Pending prescriptions (with `clawdoctor rx preview` hints)
...
export function registerDashboardCommand(program: Command): void {
  program
    ...
    .action(async (opts) => {
      ...
      const persistentDbPath = join(process.env.HOME ?? "~", ".clawdoctor", "clawdoctor.db");
      const dbPath = existsSync(persistentDbPath) ? persistentDbPath : ":memory:";
      const db = openDatabase(dbPath);

      const configPath = join(process.env.HOME ?? "~", ".clawdoctor", "config.json");
      ...
      if (dbPath === ":memory:") {
        console.log("⚠ No persistent data found. Run 'clawdoctor checkup' first or install the plugin.");
      }
...
describe("E2E Phase 2", () => {
  it("clawdoctor checkup --json includes llmAvailable field", () => {
  ...
  it("clawdoctor rx list returns empty array when no prescriptions", () => {
    const output = runPlain("rx list --json", fixtureEnv);
    expect(JSON.parse(output)).toEqual([]);
  });

  it("clawdoctor dashboard --help shows port option", () => {
